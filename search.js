...
function search() {
	...
	var json = fetch(req).then(function (response) {
		...
		for (let i = 0; i < 4; i++) {
			...
			var str3 = '<div class="display-bottommiddle"></div></div>';
		...
		for (let i = 0; i < 15; i++) {
			...
			var description = document.createTextNode(data.items[i].snippet.description.substring(0, 100) + '...');
			...
			pagediv.innerHTML = '<a onclick="showPage(1)">1</a><a onclick="showPage(2)" >2</a><a onclick="showPage(3)">3</a><a onclick="showPage(4)">4</a>';
